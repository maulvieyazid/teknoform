            ...
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col d-flex justify-content-end">
                            <a href="{{ route('majalah.create') }}" class="btn btn-primary ">
                ...
                <div class="card-body">
                    <table class="table table-striped table-responsive" id="table_master">
                        ...
                        <tbody>
                            @foreach ($semuaMajalah as $majalah)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $majalah->judul }}</td>
                                    <td>{{ $majalah->edisi }}</td>
                                    <td>
                                        <img src="{{ asset($majalah->thumbnail ?? 'images/no-photos.webp') }}"
                                            width="100" height="100">
                                    </td>
                                    <td>{{ $majalah->created_at->translatedFormat('l, d F Y H:i:s') }}</td>
                                    <td>
                                        <div class="btn-group dropstart  mb-1">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item"
                                                    href="{{ route('majalah.show', $majalah->id_majalah) }}">
                                                    <i class="bi bi-eye-fill me-50"></i> Lihat
                                                </a>
                                                <a class="dropdown-item"
                                                    href="{{ route('majalah.edit', $majalah->id_majalah) }}">
                                                    <i class="bi bi-pencil-square me-50"></i> Ubah
                                                </a>
                                                <a class="dropdown-item" href=""
                                                    onclick="event.preventDefault(); hapus(this, '{{ $majalah->judul }}')">
                                                    <i class="bi bi-trash-fill me-50"></i> Hapus
                                                    <form action="{{ route('majalah.destroy', $majalah->id_majalah) }}"
